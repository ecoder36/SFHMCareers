...
function request_add($site,$ptype,$name,$problem,$ename,$status)
{
    ...
    $query = sprintf("INSERT INTO `requests` VALUE(NULL,'%s','%s','%s','%s','%s','%s','%s','%s',NULL)",$n_site,$n_ptype,$n_name,$n_problem,$n_ename,$n_date,$n_day,$n_status);
    //echo $query;
...
function request_get_by_site_ptype_status($site,$ptype,$status)
{
    ...
    $result = request_get("WHERE `site`='$n_site' AND `ptype` = '$n_ptype' AND `status` = '$n_status'");
